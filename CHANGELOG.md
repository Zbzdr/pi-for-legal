...
- 明确 setup 和 matter 初始化只能使用当前会话中用户明确提供并授权的内容；不从历史 session、无关文件、其他项目或主目录配置补全信息。
- 将 matter 创建定义为 intake 和目录初始化步骤；生成的 matter 文件会标记用户输入和文件为待核实参考，不自动视为事实、法律结论、期限、适用法或已批准政策。
- 记录用户明确提供的 matter 请求和文件路径，便于后续核查来源。
