---
name: legal-corporate-integration-management
...
5. Read/write `<dataDir>/corporate-legal/deals/[code]/integration-tracker.yaml`.
...
Lives at `<dataDir>/corporate-legal/deals/[code]/integration-tracker.yaml`. Read `deal-context.md` for
...
```
/skill:legal-corporate-integration-management --init [--deal [code]]
```

### Step 1: Load deal context

Read `<dataDir>/corporate-legal/deals/[code]/deal-context.md`. If not found: ask for deal code name,
target company, close date, deal lead, and outside counsel. Write to
deal-context.md if it doesn't exist.

Read `<dataDir>/corporate-legal/deals/[code]/closing-checklist.yaml` if it exists. Any items marked as
...
>   automatically from `<dataDir>/corporate-legal/deals/[code]/closing-checklist.yaml`
...
Next step: run /skill:legal-corporate-integration-management --contracts to import the
...
```
/skill:legal-corporate-integration-management --contracts [--deal [code]]
```
...
```
/skill:legal-corporate-integration-management --report [--deal [code]]
```
...
```
/skill:legal-corporate-integration-management --update [--deal [code]]
```
...
Pi updates the relevant tracker entry, recalculates any downstream status
...
```
/skill:legal-corporate-integration-management --export [--format csv|table] [--section all|consents|contracts|workplan]
```
