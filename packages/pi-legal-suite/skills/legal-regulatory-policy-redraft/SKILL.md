...
1. Load the project legal profile → policy library index + practice profile.
2. Use the workflow below.
3. Gather inputs: the gap (from `/skill:legal-regulatory-gaps` output or described directly), the current approved policy text, the rule text.
...
One of:
- A `GAP-ID` from the gap tracker — load the entry from `<dataDir>/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
- A gap described in the user's message — capture the requirement, the regulation, and the affected policy.
- A diff summary pasted from `/skill:legal-regulatory-policy-diff` output.
...
If the rule text is partial or ambiguous, apply the **no silent supplement** rule from project legal profile: offer the user the options (paste full text, point at primary source, web-search-with-verify-tag, or stop), and wait.
...
This skill reads the policy library index and owners from the project legal profile. When a value it needs is empty or still `[PLACEHOLDER]`:

- **Policy owner missing:** still produce the redraft. Note in the reviewer note: "No policy owner is set for [policy] in `## Policy library`. Assign one with `/skill:legal-setup --redo` so the approval path is routable."
...
- **Gap tracker state:** this skill does NOT change the tracker. It doesn't mark the gap closed, doesn't mark it in-progress, doesn't touch `notified`. If you want a paper trail that a redraft exists, the policy owner or the user can update the gap entry with a resolution note when the redraft is applied and approved — see `/skill:legal-regulatory-gaps --close`.
- **Severity floor:** if the upstream gap is 🔴 or 🟠, the memo's Bottom line carries that severity. Silent demotion is a contradiction a reviewing lawyer cannot see. See project legal profile `## Cross-skill severity floor`.
