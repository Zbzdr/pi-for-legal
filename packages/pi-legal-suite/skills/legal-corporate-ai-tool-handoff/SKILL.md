---
name: legal-corporate-ai-tool-handoff
description: "Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust level in `the project legal profile`. Use when user says \"send to Luminance\", \"bulk review\", \"AI extraction\", or when diligence-issue-extraction hits a high-volume category."
---
# AI Tool Handoff
...
If the project legal profile says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per the project legal profile (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per the project legal profile — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per the project legal profile]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if the project legal profile says this — it's rare.)
...
**Trust level:** [per the project legal profile]
...
- It doesn't replace the tool's output with its own judgment entirely — if the project legal profile says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in the project legal profile, set at cold-start based on the team's experience with the tool.
