...
1. Load the project legal profile → case theory, key facts.
...
Two rules that govern every citation and every quotation pulled from the record into this outline. Canonical statement lives in the package's `project legal profile` shared guardrails; repeated here because an impeachment confrontation built on a misquoted prior statement or a misgrounded transcript cite collapses the impeachment.
...
- Facts from the project legal profile → key facts for us, that this witness can establish
