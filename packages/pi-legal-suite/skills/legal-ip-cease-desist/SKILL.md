...
- `/skill:legal-ip-cease-desist --send` — draft a cease-and-desist letter calibrated to your enforcement posture. Loud gate runs before delivery.
- `/skill:legal-ip-cease-desist --receive` — triage a C&D someone sent you. Produces an options memo with a recommendation.

## Instructions

1. **Read the practice profile.** Load the project legal profile. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This package needs setup before it can give you useful output. Run `/skill:legal-setup` — the C&D skill depends on your enforcement posture, approval matrix, and practice-area mix, none of which are configured yet."

2. **Check matter workspaces.** Per `## Matter workspaces`: if `Enabled` is `✗`, skip — skills use practice-level context. If enabled and there is no active matter, ask: "Which matter is this for? Run `/skill:legal-matter-workspace switch <slug>` or say `practice-level`."
...
> **External deliverable (send mode):** the drafted C&D is sent to counterparty. Do NOT include the `PRIVILEGED & CONFIDENTIAL — ATTORNEY WORK PRODUCT` header on the outgoing letter. Internal drafts, pre-send briefs, and triage memos keep the header per package config `## Outputs`.
...
- **Matter context.** Check `## Matter workspaces` in the practice-level project legal profile. If `Enabled` is `✗` (the default for in-house users), skip matter machinery — skills use practice-level context. If enabled and there is no active matter, ask: "Which matter is this for? Run `/skill:legal-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific overrides (e.g., posture override, approver override). Write outputs under `<dataDir>/matters/<matter-slug>/outputs/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
- Next step: draft a response letter — consider running it through `/skill:legal-ip-cease-desist --send` reframed as a response
...
- Respond firmly → hand off to `/skill:legal-ip-cease-desist --send` with context pre-populated as a response letter (this triggers the send-mode gate anew).
...
- Matter creation → if there isn't one and the matter is material, offer `/skill:legal-matter-workspace new <slug>` pre-populated.
