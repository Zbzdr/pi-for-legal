...
1. Load the project legal profile → watchlist, materiality threshold, feed config.
...
the project legal profile → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
Before running the pull, compare the watchlist + feed configuration in project legal profile against `references/source-catalog.md`:
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run `/skill:legal-setup --redo` to update, or edit the project legal profile directly.

Don't nag the same gap repeatedly — if the user has explicitly said "skip state AGs for now," respect that. Note state in project legal profile so it sticks.
...
- **Direct regulator RSS** — fetch and parse any RSS URLs in the project legal profile feed
...
Each item gets a materiality tier per the project legal profile:
...
| ANPR (Advance Notice of Proposed Rulemaking) | Review-worthy for **strategy**, not compliance — no imposed requirements yet, but signals direction and carries a real comment deadline. Log the comment deadline. Route to `/skill:legal-regulatory-policy-diff` only as a pre-positioning analysis, not as a gap-closure diff. |
...
- **Do** log the comment deadline to `<dataDir>/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
- **Do** include in the digest entry a line that says explicitly: "Pre-rule. Comment deadline [date]. Route to `/skill:legal-regulatory-policy-diff` only as a pre-positioning analysis (no compliance gap yet)." This primes the policy-diff skill to use its compressed pre-positioning branch rather than a full gap-closure diff.
...
- If comment tracking is enabled in the project legal profile: append to `<dataDir>/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from the project legal profile
...
The digest goes into the chat by default. **Also write it to a shareable file** whenever the output contains one or more items above FYI, unless the user's project legal profile explicitly sets `Digest output → chat only`.

**File output behavior:**

1. Look for `Digest output path` in the project legal profile. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
**Comment tracker:** [N] NPRMs with open comment decisions — run /skill:legal-regulatory-comments to review
...
This skill reads the watchlist, materiality threshold, and feed configuration from the project legal profile. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `/skill:legal-setup --redo` or edit the project legal profile and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `/skill:legal-setup --redo` or by editing the project legal profile."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `/skill:legal-setup --redo` or by editing the project legal profile."
...
End with the next-steps decision tree per project legal profile `## Outputs`. Customize the options to what this skill just produced — the five default branches (draft the X, escalate, get more facts, watch and wait, something else) are a starting point, not a lock-in. The tree is the output; the lawyer picks.

## What this skill does not do

- Read every item in full. It classifies and enriches; deep reading is for the
  items that survive the filter.
- Change the materiality threshold. If the filter is wrong, edit the project legal profile.
