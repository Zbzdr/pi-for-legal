...
1. Load the project legal profile → DSAR process (systems list, verification method, SLA).
...
Read the project legal profile → `## DSAR process`. That section has:
...
Per the method in the project legal profile. Common approaches:
...
Walk the systems list from the project legal profile. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from the project legal profile `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product — keep those separate and prepend the work-product header per the project legal profile `## Outputs` (which differs by user role — see `## Who's using this`).
...
Per the project legal profile → Escalation table, escalate when:
...
If the project legal profile → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
