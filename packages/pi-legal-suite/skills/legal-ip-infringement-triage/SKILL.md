...
1. Read the project legal profile. If it
   contains `[PLACEHOLDER]`, stop and direct to `/skill:legal-setup`.
...
8. End with recommended next steps, the non-lawyer gate if the role is
   ...
   draft the C&D via `/skill:legal-ip-cease-desist` or the takedown via
   `/skill:legal-ip-takedown`. Do not draft automatically.
...
```
/skill:legal-ip-infringement-triage "competitor launched a tool called APEXSEED in class 9 — we have APEXLEAF registered in class 9; likely confusion?"
```

```
/skill:legal-ip-infringement-triage "former engineer took notes on our model architecture to a competitor — possible trade secret?"
```
...
**The loudest guardrail in the package. Say this at the top of every output. Do
...
Read the project legal profile. Pull:
...
If the user says "provisional," run the infringement triage normally using these generic defaults: middle risk appetite, lawyer role, governing jurisdiction from the task/profile, no playbook (do the full analysis rather than matching against a position list). Tag the reviewer note and every finding block with `[PROVISIONAL]`. At the end of the output, append:
...
**Route to `/skill:legal-ip-fto-triage` for the detailed framework.** This mode is the
...
invalidity contentions, run `/skill:legal-litigation-claim-chart`. This triage's
...
Prepend the work-product header from the project legal profile `## Outputs`.
...
- [routing per the project legal profile
...
`<dataDir>/matters/<matter-slug>/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`<dataDir>/ip-legal/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`
...
> Want me to draft a cease-and-desist on this? Run `/skill:legal-ip-cease-desist`.
...
> Want me to prepare a DMCA takedown? Run `/skill:legal-ip-takedown`.
...
- **Draft the C&D, takedown, or complaint.** Those are separate skills
  (`/skill:legal-ip-cease-desist`, `/skill:legal-ip-takedown`) gated by the approval
