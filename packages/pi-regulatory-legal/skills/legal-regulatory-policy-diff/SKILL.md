...
1. Load the project legal profile → policy library index.
...
the project legal profile → policy library index (policies, locations, owners).
...
- Note the comment deadline and the team's comment-decision owner from the project legal profile.
...
[other-policy-1] and [other-policy-2] — rerun `/skill:legal-regulatory-policy-diff` against those.
...
This skill reads the policy library index from the project legal profile. When the index is empty or still `[PLACEHOLDER]`:

- **Policy library empty:** flag every requirement as "no policy match" by default and append to the output: "The policy library in your configuration is empty, so every requirement is flagged as a new-policy gap. If you have policies that address these requirements, add them to the library with `/skill:legal-setup --redo` or by editing the project legal profile, then re-run the diff."
- **Owner missing for a matched policy:** leave the Owner cell blank in the summary and append: "Policy owners aren't set for [list]. Assign them with `/skill:legal-setup --redo` or by editing the policy library in the project legal profile so gap-surfacer can route."
