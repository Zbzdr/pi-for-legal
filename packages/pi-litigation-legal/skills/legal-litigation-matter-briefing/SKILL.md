...
> "I don't see [matter slug] in the matter log. Run `/skill:legal-litigation-matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a briefing on a matter that hasn't been intaken — the conflicts check is the gate."
...
If `last_updated > 30 days ago`: flag at the top AND suggest running `/skill:legal-litigation-matter-update [slug]` after the meeting to capture whatever's discussed.
