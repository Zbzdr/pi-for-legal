...
1. Load the project legal profile → risk calibration (for triage), landscape (for context, conflicts method), stakeholders (for who to loop in).
...
Before going further, run the conflicts step per the project legal profile → Conflicts clearance.

- **Status:** `cleared | pending | not-run | waived`
- **Method:** match what the project legal profile declares (`corporate-legal | outside-counsel | system-check | informal | other`). If the declared method is `informal`, say so — the record still captures that a counsel's-judgment check was the basis.
...
- `not-run` → **STOP. This is a gate.** The skill will not create `matter.md`, `history.md`, or a `_log.yaml` entry until the conflicts posture is resolved. Three acceptable paths:
  ...
  **Path 1 — Run conflicts now.** Pause this intake. Clear per the project legal profile Conflicts clearance. Return with `status: cleared` or `status: waived` with rationale.

  **Path 2 — Mark pending with owner + due date.** Allowed only when the project legal profile Conflicts clearance declares parallel-intake acceptable. Capture: who is running conflicts, when they're expected to return, what entities they're checking. Intake proceeds; matter row carries `conflicts.status: pending`; `/portfolio-status` flags it every run; `/matter-update` re-prompts until resolved.
...
- Severity: high | medium | low (reference the the project legal profile severity bands)
- Likelihood: high | medium | low (reference the the project legal profile likelihood bands)
...
If the risk calibration in the project legal profile is thin, don't fake precision. Use the user's gut and note the thinness.

### 5. Materiality

Against the house thresholds in the project legal profile:
...
From the project legal profile landscape — which internal stakeholders are involved?
- Business lead
...
- If no and this is active litigation or reasonably anticipated: flag urgently; offer to run `/skill:legal-litigation-legal-hold [slug] --issue` after intake completes.
