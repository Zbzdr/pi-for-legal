...
1. Load `<dataDir>/litigation-legal/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load the project legal profile → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in the project legal profile; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `<dataDir>/litigation-legal/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `<dataDir>/litigation-legal/demand-letters/[slug]/checklist.md` (post-send checklist).
...
Demand letters are advocacy, and every quoted line from a contract, an email, or a prior communication becomes an assertion the counterparty will test. Canonical statement in the package's `project legal profile` shared guardrails; repeated here.
...
Check the project legal profile → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply the project legal profile privilege conventions exactly.

## Output

### Primary: `<dataDir>/litigation-legal/demand-letters/[slug]/draft-v[N].docx`
...
### `<dataDir>/litigation-legal/demand-letters/[slug]/checklist.md` — the post-send checklist

```markdown
[WORK-PRODUCT HEADER — per package config ## Outputs — differs by role; see `## Who's using this`. This header applies to the internal checklist file; the outgoing letter does NOT carry it.]
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
...
If material: `/skill:legal-litigation-matter-intake` with `source: demand-letter` pre-populated from this intake.
```
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ the project legal profile medium-severity band
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `<dataDir>/litigation-legal/demand-letters/` only.
