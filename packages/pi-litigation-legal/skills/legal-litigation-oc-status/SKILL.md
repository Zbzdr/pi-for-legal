...
5. Write markdown to `<dataDir>/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`.
6. If Gmail MCP authenticated: create Gmail drafts. Else: markdown-only, note in summary.
7. Write `<dataDir>/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md` — what ran, what was skipped and why.
...
**Subject:** per house convention (from the project legal profile outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per the project legal profile budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From the project legal profile signer default for OC directives.]
```

Adapt tone per the project legal profile outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `<dataDir>/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`
...
**From:** [signer name / email from the project legal profile]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (the project legal profile privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
After processing all matters, write `<dataDir>/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md`:
...
This skill is designed to run weekly. Automated scheduling requires a scheduled-tasks integration that is not bundled with the package. To run weekly, set a recurring reminder to invoke `/skill:legal-litigation-oc-status` — e.g., Monday morning on your calendar.

Ad-hoc: `/skill:legal-litigation-oc-status` any time. `/skill:legal-litigation-oc-status --slug=foo` for a single matter.
...
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match the project legal profile.
