...
1. Load the project legal profile → demand-letter practice, landscape, risk calibration.
...
5. Write `<dataDir>/litigation-legal/demand-letters/[slug]/intake.md`.
6. Confirm with user: "Intake saved. Run `/skill:legal-litigation-demand-draft [slug]` when ready."
...
- Desired outcome dollar value ≥ the medium-severity band from the project legal profile risk calibration
- Counterparty is a customer, competitor, or frequent adversary per the project legal profile landscape
- User ran with `--full`
...
`[type]-[counterparty-short]-[yyyy-mm]`. Confirm uniqueness in `<dataDir>/litigation-legal/demand-letters/`.

### `<dataDir>/litigation-legal/demand-letters/[slug]/intake.md`
...
End with:
> Intake saved. When ready: `/skill:legal-litigation-demand-draft [slug]`
...
- Run the conflicts check. If the counterparty is a customer or known entity, flag that this should clear conflicts (per the project legal profile) before sending — but the check itself lives in the matter-intake workflow or outside this skill.
