---
name: legal-employment-internal-investigation
...
Prepend the work-product header from the project legal profile → `## Outputs` (it differs by user role — see `## Who's using this`). Every file, log, memo, and summary produced by this skill opens with that header.
...
Read the project legal profile → escalation table, any investigation protocols noted.

---

## Mode 1: Open a new matter

Triggered by `/skill:legal-employment-investigation-open` or "open an investigation"
...
`<dataDir>/employment-legal/investigation-[matter-slug]/log.yaml`:
...
`<dataDir>/employment-legal/investigation-[matter-slug]/sources-checklist.yaml`:

Generated from the investigation type. See sources checklist templates below.

`<dataDir>/employment-legal/investigation-[matter-slug]/documents-reviewed.yaml`:
...
`<dataDir>/employment-legal/investigation-[slug]/sources-checklist.yaml`.

---

## Mode 2: Add data

Triggered by `/skill:legal-employment-investigation-add` or "add to the [matter]
investigation" or when the attorney pastes documents or interview notes.

### Step 1 — Identify the matter

If multiple investigation folders exist in `<dataDir>/employment-legal/`, ask which matter this
...
Triggered by `/skill:legal-employment-investigation-query` or any question
...
Triggered by `/skill:legal-employment-investigation-memo` or "draft the memo"
or "update the memo".
...
Write the draft to `<dataDir>/employment-legal/investigation-[slug]/memo.md`.
...
Triggered by `/skill:legal-employment-investigation-summary` or "draft a
...
- Header: "[WORK-PRODUCT HEADER — per package config ## Outputs — differs by role; see `## Who's using this`]"
...
**Before producing a summary, memo, or content intended for an external response (EEOC/DFEH/state agency charge response, plaintiff's-counsel demand letter response, regulator response, or any formal complaint reply):** Read `## Who's using this` in the project legal profile. If the Role is **Non-lawyer**:
