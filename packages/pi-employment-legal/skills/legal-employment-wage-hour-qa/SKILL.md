---
name: legal-employment-wage-hour-qa
...
1. Load the project legal profile → jurisdictional footprint.
...
the project legal profile → jurisdictional footprint. If the question doesn't specify a
...
- "Can we classify this person as a contractor?" — Route to
  `/skill:legal-employment-worker-classification` if the facts are not already clear.
