...
1. Load the project legal profile → `## Entity Management` (entity table, jurisdictions, registered agent).
...
3. Read/write `<dataDir>/corporate-legal/entities/compliance-tracker.yaml`.
...
is a file you own, Pi updates it on command, and you export it when you need
to share it.
...
Lives at `<dataDir>/corporate-legal/entities/compliance-tracker.yaml`. Structure:
...
Read the project legal profile → `## Entity Management` → Entity table. If the entity table
...
Generate `<dataDir>/corporate-legal/entities/compliance-tracker.yaml` with all entities and their
...
Run /skill:legal-corporate-entity-compliance --report to see what's due.
```
...
```
/skill:legal-corporate-entity-compliance --report [--days 30|60|90|180]
```
...
If the tracker covers more than ~10 entities, or any time the user asks: offer the dashboard (see project legal profile `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by filing status (overdue / due soon / filed / unknown), counts by good-standing state, and a sortable entity table with jurisdiction, filing type, and next due date.
...
**Before directing or confirming a filing:** Read `## Who's using this` in the project legal profile. If the Role is **Non-lawyer**:
...
```
/skill:legal-corporate-entity-compliance --update
```

Attorney tells Pi what was filed:
...
```
/skill:legal-corporate-entity-compliance --update --from-report
```

User uploads a CT Corp, National Registered Agents, or similar compliance
report (PDF, CSV, or Excel). Pi reads it and updates matching entities:
...
```
/skill:legal-corporate-entity-compliance --sweep
```
...
```
/skill:legal-corporate-entity-compliance --audit
```
...
- Based on the project legal profile entity table: are there states in the company's
  ...
  Pi can flag the question but cannot determine presence independently.

**Intercompany agreement gaps:**
- From the project legal profile: if intercompany agreements are marked as partial or no,
...
POTENTIAL GAPS
  Foreign qualification: [flag question — confirm operational presence in:]
    [list of states from the project legal profile footprint not in tracker as qualified]
  Intercompany agreements: [status from the project legal profile]
...
```
/skill:legal-corporate-entity-compliance --export [--format csv|table]
```
