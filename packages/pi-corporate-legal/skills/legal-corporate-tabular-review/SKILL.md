...
When `--template ma-diligence` is requested, read `references/ma-diligence-columns.md`; also load the output-format reference selected by the user.

1. Load the project legal profile → diligence structure, thresholds, house format.
...
```
/skill:legal-corporate-tabular-review
/skill:legal-corporate-tabular-review --schema .review-schema.yaml --docs ./vdr/02-Contracts/
/skill:legal-corporate-tabular-review --template ma-diligence
```
...
- the project legal profile → diligence structure, materiality thresholds, house format preferences
- `<dataDir>/corporate-legal/deals/[code]/deal-context.md` if working a specific deal
...
Prepend the work-product header from the package config `## Outputs` as a top row. Alongside it, include a distribution note:
