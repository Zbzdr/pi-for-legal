---
name: legal-corporate-board-minutes
...
- the project legal profile → `## Board & Secretary` section:
...
- If the project legal profile has no minutes format: run cold-start first. Do not proceed with a generic format.
...
**Directors present:**
- Pull from board composition in the project legal profile as the starting point
...
Use the house format from the project legal profile. Do not default to a generic format. The seed minutes are the template — replicate the structure, the header, the resolution language, the level of discussion detail.
...
**Previous minutes:**
Standard language: approval of minutes from prior meeting. Pull date of prior meeting from the project legal profile board calendar if available, otherwise leave as [DATE OF PRIOR MEETING].
...
RESOLVED, that [resolution text in house language from the project legal profile].
```
...
**Before adopting minutes as final:** Read `## Who's using this` in the project legal profile. If the Role is **Non-lawyer**:
...
Produce the full draft. The minutes themselves are a corporate record, not privileged; do not apply the work-product header to the minutes as circulated. The drafting notes, placeholder flags, and review checklist below are work product — prepend the work-product header from the project legal profile `## Outputs` (it differs by user role — see `## Who's using this`):
...
For drafting written consents in lieu of a meeting, use `/skill:legal-corporate-written-consent`. That skill handles precedent search, state-law confirmation, and the scope warning for major one-off actions.
