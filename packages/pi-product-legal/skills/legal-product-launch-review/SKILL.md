---
name: legal-product-launch-review
...
> **Attribution:** Adapted from Anthropic's `claude-for-legal/product-legal` at revision `4a6c651889c97cc9140580363c73e0eb17379c2b` under Apache-2.0 and modified for Pi. See the package `NOTICE`.
If the profile has no configured launch framework, read `references/seven-category-framework.md` before performing the category walk.

1. Load the project legal profile → framework + calibration. Stop if placeholders.
...
```
/skill:legal-product-launch-review PROJ-1234
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level project legal profile. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/skill:legal-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `<dataDir>/product-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Destination check

Before producing output, check where it's going. If the user has named a destination (a channel, a distribution list, a counterparty, "everyone"), ask whether it's inside the privilege circle. Public channels, company-wide lists, counterparty/opposing counsel, vendors, and clients (for work product) waive the protection. When the destination looks outside the circle, flag it and offer (a) the privileged version for legal only, (b) a sanitized version for the broader channel, or (c) both — don't silently apply a privileged header and then help paste it somewhere the header won't protect it. See the canonical `## Shared guardrails → Destination check` in this package's project legal profile.

## Purpose

Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per the project legal profile), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.

## Load calibration

Read the project legal profile:
...
If AI component detected → flag it, then run `/skill:legal-ai-use-case-triage [feature]`
...
**Calibration:** [per the config project legal profile — this is usually an FYI / usually needs X / usually blocks]
...
For each finding, check against the calibration table in the project legal profile:
...
Format per the project legal profile → Launch review process → output format. Prepend the work-product header from the project legal profile `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:

```markdown
[WORK-PRODUCT HEADER — per package config ## Outputs]
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
>
...
**Output 1 — Privileged launch review memo.** The full analysis assembled in Step 5: work-product header, bottom line, findings by category with risk rationale, action items, escalations, notes for next time, citation check. This is internal legal work product. Keep it in your matter file (Drive, DMS, or wherever the project legal profile says review docs go). Distribute only to people inside the privilege circle.
...
- **To privacy:** If the launch touches personal data, run `/skill:legal-privacy-use-case-triage [feature]`. If triage returns PIA REQUIRED or DPIA MANDATORY, run `/skill:legal-privacy-pia-generation [feature]`. Don't just note "PIA needed" — trigger it.
- **To AI governance:** If an AI component was detected in Step 2, run `/skill:legal-ai-use-case-triage [feature]`. If triage returns CONDITIONAL, run `/skill:legal-ai-impact-assessment [feature]`. If a new AI vendor is involved, run `/skill:legal-ai-vendor-review [vendor agreement]`.
...
- It doesn't retroactively calibrate. If this launch turns out fine (or badly) in a way that should update the calibration table, a human updates the project legal profile.
