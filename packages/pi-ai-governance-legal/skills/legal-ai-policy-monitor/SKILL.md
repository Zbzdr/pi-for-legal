...
1. Read the project legal profile → outputs folder path, AI policy document, last sweep date.
...
6. Flag any use cases in outputs not yet added to the the project legal profile registry.
7. Present results to the human. Only after acknowledgment, update `Last policy sweep` and `gaps_found` in the project legal profile.

**Direct query mode** (with description argument):
1. Read the project legal profile → current policy commitments, use case registry, actual policy document.
...
**Recurring runs:**
Set up a recurring reminder in your own scheduler to run `/skill:legal-ai-policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this package.

```
/skill:legal-ai-policy-monitor
/skill:legal-ai-policy-monitor "We want to use AI to automatically flag expense reports for review"
```
...
> enable the crawl sweep, run `/skill:legal-setup` and provide the outputs
> folder path."

Read the actual AI or acceptable use policy document from the path in `## Outputs`
→ **AI policy document**. The commitments in the project legal profile are a summary; the actual
...
**Do not update `Last policy sweep` or `gaps_found` automatically.** After the sweep results are presented, wait for the human to acknowledge them ("sweep acknowledged," "results reviewed," or equivalent). Only then update the project legal profile:
...
- Extract: vendor added, data use terms agreed to, any AI-specific provisions
  accepted that differ from standard positions in the project legal profile
...
*This sweep is derived from AIAs, triage results, and vendor AI reviews that carry the package's privilege/confidentiality marking. The sweep inherits that status. Distribute deliberately — forwarding gap findings outside the privilege circle can waive privilege on the underlying assessments.*
...
[Any use cases approved since the last sweep that aren't yet in the the project legal profile
...
[If this use case isn't in the registry: "Add to the project legal profile → Use case registry:"]
```
...
The weekly sweep is designed to run on a recurring cadence. Set up a recurring reminder in your own scheduler to run `/skill:legal-ai-policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this package.
