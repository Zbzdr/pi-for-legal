...
1. Read the project legal profile. Confirm impact assessment house style is populated.
...
6. Policy diff against the project legal profile AI policy commitments.
7. Output: assessment doc + conditions list + handoff flags (privacy PIA, vendor review if needed).

```
/skill:legal-ai-impact-assessment "AI résumé screening for HR"
```
...
- Is the use case in the elevated or high governance tier per the project legal profile?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from the project legal profile (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
**Fast track vs. full assessment:** the project legal profile defines what qualifies for abbreviated treatment. If the project legal profile doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in the project legal profile → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from the project legal profile.** If none was captured, use this default:
...
|---|---|---|
| [commitment from the project legal profile AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Privacy review required?** [Yes — run `/skill:legal-privacy-pia-generation`, if the package is installed /
No]
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in the project legal profile.
Common drift:
...
- **To privacy:** If personal data is involved, flag: "Run `/skill:legal-privacy-pia-generation [system name]` in parallel, if the package is installed — the AIA doesn't substitute for a PIA."
- **To vendor-ai-review:** If a new vendor is involved, flag: "If there's no AI addendum reviewed for [vendor], run `/skill:legal-ai-vendor-review` before production."
