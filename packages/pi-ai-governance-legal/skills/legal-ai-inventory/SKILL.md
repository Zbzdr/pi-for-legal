...
1. **Read the config.** Read
   ...
   to `/skill:legal-setup` first.

2. **Read the inventory.** Inventory lives at
   `<dataDir>/ai-governance-legal/ai-systems.yaml`.
...
`/skill:legal-ai-inventory classify <id>`."
...
2. "Want to run `/skill:legal-ai-impact-assessment` to produce a full
   impact assessment?"
...
`/skill:legal-ai-impact-assessment` for the formal impact assessment
if needed.
...
- **Flag substantial modification.** Whenever a system is modified beyond
  configuration, prompt the user to re-run `/skill:legal-ai-inventory classify` —
