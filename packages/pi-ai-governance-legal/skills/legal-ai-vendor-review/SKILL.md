...
1. Read the project legal profile. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. the project legal profile.
7. Output: bottom line, term-by-term, recommended redlines, if-they-won't-move routing.

```
/skill:legal-ai-vendor-review openai-enterprise-agreement.pdf
```
...
Read the project legal profile → `## Vendor AI governance`. Also read `## AI policy commitments`
— vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If the project legal profile contains `[PLACEHOLDER]`, surface this bounce:
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in the project legal profile → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
> **Our position:** [from the project legal profile]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against the project legal profile positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per the project legal profile.
...
Cross-check the vendor's terms against our AI policy commitments in the project legal profile.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from the project legal profile, or "escalate — outside fallback"
...
4. **Hosted knowledge base / RAG source** (e.g., a vector database, a third-party data corpus, a retrieval service) — the data Pi reads from
...
- It doesn't review the DPA provisions of the same agreement — run
  `/skill:legal-privacy-dpa-review`, if the package is installed, for that.
