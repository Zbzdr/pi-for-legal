...
1. Read the project legal profile. If the practice profile is unpopulated, stop and direct to `/skill:legal-setup`.
...
```
/skill:legal-ai-policy-starter
/skill:legal-ai-policy-starter "we need an AI policy for our 30-lawyer firm"
/skill:legal-ai-policy-starter "update our existing policy for the 2026 state AI laws"
```
...
up doing with Copilot and Pi for Work. This skill produces a **draft** policy
...
3. **Training materials** — once the policy is adopted, `/skill:legal-ai-impact-assessment` can be used to produce per-use-case training notes.
4. **Vendor sweep** — once the policy is adopted, `/skill:legal-ai-vendor-review` should be run against the vendors the policy references to check conformance.
5. **Gap check against new regulation** — pair with `/skill:legal-ai-reg-gap-analysis` to test the draft against a specific regulation or guidance before adoption.
