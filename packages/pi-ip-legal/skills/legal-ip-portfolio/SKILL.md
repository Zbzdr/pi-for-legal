...
1. **Follow the workflow below** and read
   `<dataDir>/ip-legal/portfolio.yaml`.
...
3. **`--report [--days N]`:** Mode 2. Change the window with `--days`
   ...
   per project legal profile → Outputs. Always close with the verification caveat.
...
```
/skill:legal-ip-portfolio --report --days 180
```

```
/skill:legal-ip-portfolio --add
```

```
/skill:legal-ip-portfolio --update
```

```
/skill:legal-ip-portfolio --audit
```
...
Lives at `<dataDir>/ip-legal/portfolio.yaml`.
Structure:
...
Run /skill:legal-ip-portfolio --report to see what's due.
```
...
```
/skill:legal-ip-portfolio --report [--days 30|60|90|180]
```
...
Output (prepend work-product header per the project legal profile → Outputs):
...
If the report lists more than ~10 assets, or any time the user asks: offer the dashboard (see project legal profile `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by registration status (live / in grace / lapsed / pending), a deadline timeline, and a sortable portfolio table with jurisdiction, type, and next-action date.
...
`## Who's using this` in the project legal profile. If the Role is **Non-lawyer**:
...
**Unwatched assets**
- Any registered marks not on the watch list in project legal profile → Brand protection?
...
The `ip-renewal-watcher` agent in this package runs this skill on a schedule
(weekly by default) and posts the Mode 2 report to the channel named in
project legal profile → Renewal alerts. If 🔴 items appear (grace / lapsed), the agent
