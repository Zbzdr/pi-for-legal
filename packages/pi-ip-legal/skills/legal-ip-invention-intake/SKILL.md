...
1. Read the project legal profile. If it
   contains `[PLACEHOLDER]`, stop and direct to `/skill:legal-setup`. If the
...
```
/skill:legal-ip-invention-intake "a new cache-eviction algorithm that uses a learned model rather than LRU; conceived Q1 this year, not yet disclosed, engineering prototype in internal staging"
```
...
project legal profile. If `Enabled` is `✗` (the default for in-house users), skip the rest
...
the project legal profile.** If it is
missing or still contains placeholders, stop and run `/skill:legal-setup`. The
...
**Never do a prior-art search in this skill.** A an available web-search capability for "does this
