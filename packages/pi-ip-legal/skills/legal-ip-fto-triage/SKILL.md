...
```
/skill:legal-ip-fto-triage "an on-device speech recognition model for consumer wearables, US launch first"
```
...
Before running triage, read the project legal profile. Pull:
...
If the project legal profile contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
Prepend the work-product header from the project legal profile `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
- **Issue an FTO opinion.** Ever. The loudest guardrail in the package.
...
- **Draft patent claims.** This package does not go there; route to prosecution
  counsel.
- **Assess damages exposure.** Damages modeling is an expert's job.
- **Handle trade-secret or trademark analysis** — use `/skill:legal-ip-infringement-triage`
  with the right mode.
