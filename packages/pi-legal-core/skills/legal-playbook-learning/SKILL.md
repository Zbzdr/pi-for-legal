...
Read `.pi/legal-workbench/config.json`, resolve the profile and `dataDir`, and use:

```text
<dataDir>/learning/deviations.jsonl
<dataDir>/learning/pending-playbook-proposals.md
<dataDir>/learning/decisions.jsonl
```

Show exact paths before the first write. Never copy contract text, personal data, client confidences, or privileged analysis into the learning log unless necessary; prefer a clause category, normalized outcome, source matter slug, and source-file anchor.
