---
name: legal-privacy-pia-generation
...
---
# /skill:legal-privacy-pia-generation
...
1. Load the project legal profile → PIA house style (trigger, structure, depth, sign-off).
...
```
/skill:legal-privacy-pia-generation "Location sharing feature"
```
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read the project legal profile → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in the project legal profile → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read the project legal profile → `## PIA house style`. That has:
...
If the seed PIA structure is in the config project legal profile, **use it**. The point is that this PIA looks like the other PIAs this team produces, not like a generic one.

## Step 0: Is a PIA needed?

Check the trigger criteria in the project legal profile. That is the team's house answer.
...
**Use the seed PIA structure from the config project legal profile.** If none was captured, use this default. Prepend the work-product header from the project legal profile `## Outputs` (it differs by user role — see `## Who's using this`).
...
|---|---|---|
| [commitment from config project legal profile privacy policy section] | 🟢 / 🟡 | |
...
Every PIA should cross-check against the privacy policy commitments in the project legal profile. The common drift:
...
- **To the sign-off process:** Per the project legal profile → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in the project legal profile. If the Role is Non-lawyer:
