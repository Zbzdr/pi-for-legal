...
1. Read the project legal profile. Confirm privacy practice is configured — if not, stop and direct to setup.
...
4. Output: classification (PROCEED / PIA REQUIRED / DPIA MANDATORY / STOP), reasoning, conditions table if required, cross-package handoffs.
5. Offer to continue into PIA generation if assessment is required.

```
/skill:legal-privacy-use-case-triage "New feature that uses behavioral data to personalize content recommendations"
```
...
Before triaging, always read the project legal profile. The PIA trigger criteria, regulatory
...
Read the project legal profile → `## PIA house style` → Trigger criteria. Apply them.
...
Read the project legal profile → `## Privacy policy commitments`. Check the proposed activity
...
`/skill:legal-privacy-pia-generation [activity]`
...
> required in addition to a PIA. Use `/skill:legal-ai-impact-assessment [activity]`
...
> Use `/skill:legal-product-launch-review` — it will detect the privacy component
> and route to this package."
